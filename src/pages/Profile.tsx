...
const Profile: React.FC<ProfileProps> = ({ onUnlock }) => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      toast({ title: ('profile Picture Set Success') });

      try {
        setUploading(true);
        await uploadToDrive(file, file.name);
        toast({ title: ('upload Drive Success') });
      } catch {
        toast({ title: ('uploadFailed'), variant: 'destructive' });
      } finally {
        setUploading(false);
      }
    };
  ...
  const handleRemovePhoto = () => {
    ...
    toast({ title: ('profile PicRemoved') });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-10">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="relative group w-32 h-32">
              ...
              <button
                ...
                className="absolute bottom-0 right-0 p-2 rounded-full bg-primary text-primary-foreground opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      {showSettings && <Settings onUnlock={onUnlock} />}
    </div>
  );
};
